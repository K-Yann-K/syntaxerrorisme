    ...
    <header className="bg-swiss-coffee-300 flex items-center justify-between pt-3.5 pb-3.5 sm:pl-5 sm:pr-5 md:pl-15 md:pr-15 shadow-sm">{/* sm:px-5 md:px-15*/}
      ...
      <nav className="flex items-center text-xs sm:text-lg sm:gap-12">
        ...
        <div className="flex items-center gap-2">
          <button ref={eduButtonRef} onClick={() => setEduOpen((v) => !v)} aria-expanded={eduOpen} aria-haspopup="menu"
            onMouseEnter={() => setEduHover(true)} onMouseLeave={() => setEduHover(false)}
          ...
          >
            ...
            <svg
              viewBox="0 0 20 20"
              fill="currentColor"
              className={`-mr-1 h-5 w-5 text-gray-600 transition-transform duration-200 ${eduOpen ? "rotate-180" : ""} ${eduHover ? "scale-110 text-gray-950" : ""}`}
            >
              <path
                fillRule="evenodd"
                clipRule="evenodd"
